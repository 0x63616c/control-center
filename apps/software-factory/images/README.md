...
Seven images, one path filter (`apps/software-factory/**`), all amd64-only. The
...
docker build --platform linux/amd64 \
  -f apps/software-factory/images/run-worker/Dockerfile \
  -t sf-run-worker:local .
apps/software-factory/images/run-worker/smoke.sh sf-run-worker:local
```

...
// CredentialFileSource returns a current, refresh-token-free Codex document.
type CredentialFileSource interface {
...
func NewManagedCredentialSource(source CredentialFileSource) (*ManagedCredentialSource, error) {
	if source == nil {
...
func (s *ManagedCredentialSource) Credential(ctx context.Context) (Credential, error) {
	file, err := s.source.SandboxCredentialFile(ctx)
	if err != nil {

...
const (
	// credentialsPath is private to the credentialed repository container. The
	// model-controlled tools container shares /work, but not this home directory.
	credentialsPath      = "/home/sandbox/.git-credentials"
	privatePushDirPrefix = "/tmp/software-factory-push."
	pushCleanupTimeout   = 30 * time.Second
)
...
// credentialHelperValue is kept separate so every fixed repository operation
// uses exactly one spelling of the private credential store.
const credentialHelperValue = "store --file=" + credentialsPath

var isolatedGitEnvironment = []string{
	"env",
...
// CloneRepo checks the ticket's repository out at work.RepoDir, creates the
// run branch, and publishes its initial head. Every credentialed command runs
// in the repository container before model-controlled tools begin.
//
...
func (s *Sandboxes) CloneRepo(ctx context.Context, sandbox work.SandboxID, cloneURL string, credential work.SandboxCredential) error {
	...
	// Written unconditionally so an activity retry always has a usable token.
	...
	// The identity is non-secret and lets the agent create attributed commits.
	if err := s.configureCheckoutIdentity(ctx, sandbox, credential); err != nil {
		return err
	}

	return s.pushBranch(ctx, sandbox, cloneURL, branch)
}

// PushRepo publishes the committed branch from the credentialed repository
// sidecar using only the supplied authoritative URL, branch, and fresh token.
func (s *Sandboxes) PushRepo(ctx context.Context, sandbox work.SandboxID, cloneURL string, credential work.SandboxCredential) error {
	if cloneURL == "" {
		return fmt.Errorf("pushing from sandbox %s: no repository url was configured: %w", sandbox, work.ErrPermanent)
	}
	branch, err := s.sandboxBranch(ctx, sandbox)
	if err != nil {
		return err
	...
	return s.publishBranch(ctx, sandbox, cloneURL, branch)
}
...
func (s *Sandboxes) sandboxBranch(ctx context.Context, sandbox work.SandboxID) (string, error) {
	...
	code, err := s.repositoryExec(ctx, sandbox, argv, nil, &out, &stderr)
	if err != nil {
...
func (s *Sandboxes) writeCredentials(ctx context.Context, sandbox work.SandboxID, credential work.SandboxCredential) error {
	if err := s.writeRepositoryCredential(ctx, sandbox, credentialsPath, []byte(credentialLine(credential.Token))); err != nil {
...
// configureCheckoutIdentity writes only non-secret author metadata. The
// checkout deliberately receives no credential helper: model tools may read
// and edit its local Git configuration.
...
func (s *Sandboxes) currentBranch(ctx context.Context, sandbox work.SandboxID) (branch string, ok bool, err error) {
	...
	code, execErr := s.repositoryExec(ctx, sandbox, argv, nil, &out, &stderr)
	if execErr != nil {
...
// pushBranch publishes the initial, pre-agent branch through a fixed URL and
// private helper. It runs before model-controlled tools can alter the checkout.
func (s *Sandboxes) pushBranch(ctx context.Context, sandbox work.SandboxID, cloneURL, branch string) error {
	return s.runExpecting0(ctx, sandbox, "pushing this run's branch", []string{
		"git", "-c", "core.hooksPath=/dev/null", "-c", "credential.helper=", "-c", credentialHelper,
		"-C", work.RepoDir, "push", "--set-upstream", "--", cloneURL, branch + ":refs/heads/" + branch,
	})
}

// publishBranch copies HEAD through a bundle into a private bare repository,
// then publishes from that repository. The model may edit the shared
// checkout's Git config; no command that can reach the network runs from it.
func (s *Sandboxes) publishBranch(ctx context.Context, sandbox work.SandboxID, cloneURL, branch string) (err error) {
	current, ok, err := s.currentBranch(ctx, sandbox)
	if err != nil {
		return err
	}
	if !ok || current != branch {
		return fmt.Errorf("publishing sandbox %s: checkout branch %q is not the run branch %q: %w", sandbox, current, branch, work.ErrPermanent)
	}
	privateDir, err := s.makePrivatePushDir(ctx, sandbox)
	if err != nil {
		return err
	}
	defer func() {
		cleanupCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), pushCleanupTimeout)
		defer cancel()
		cleanupErr := s.runExpecting0(cleanupCtx, sandbox, "removing private publication state", []string{"rm", "-rf", "--", privateDir})
		if cleanupErr != nil {
			err = errors.Join(err, cleanupErr)
		}
	}()

	bundlePath := path.Join(privateDir, "source.bundle")
	privateRepo := path.Join(privateDir, "repository.git")
	if err := s.runExpecting0(ctx, sandbox, "bundling the committed run branch",
		isolatedGit("-c", "core.hooksPath=/dev/null", "-C", work.RepoDir,
			"bundle", "create", bundlePath, "HEAD")); err != nil {
		return err
	}
	if err := s.runExpecting0(ctx, sandbox, "initializing the private publication repository",
		isolatedGit("-c", "core.hooksPath=/dev/null", "init", "--bare", privateRepo)); err != nil {
		return err
	}
	if err := s.runExpecting0(ctx, sandbox, "loading the bundle into the private publication repository",
		isolatedGit("-c", "core.hooksPath=/dev/null", "-C", privateRepo,
			"fetch", "--no-tags", "--force", "--", bundlePath, "HEAD:refs/heads/publish")); err != nil {
		return err
	}

	push := append(append([]string(nil), isolatedGitEnvironment...), "GIT_TERMINAL_PROMPT=0")
	push = append(push,
		"git", "-c", "core.hooksPath=/dev/null", "-c", "credential.helper=", "-c", credentialHelper,
		"-C", privateRepo, "push", "--", cloneURL,
		"refs/heads/publish:refs/heads/"+branch,
	)
	if err := s.runExpecting0(ctx, sandbox, "pushing this run's branch", push); err != nil {
		return err
	}
	return nil
}

func (s *Sandboxes) makePrivatePushDir(ctx context.Context, sandbox work.SandboxID) (string, error) {
	var out, stderr bytes.Buffer
	argv := []string{"mktemp", "-d", "-p", "/tmp", "software-factory-push.XXXXXXXXXX"}
	code, err := s.repositoryExec(ctx, sandbox, argv, nil, &out, &stderr)
	if err != nil {
		return "", fmt.Errorf("creating private publication state in sandbox %s: %w", sandbox, err)
	}
	if code != 0 {
		return "", exitCodeError(sandbox, "creating private publication state", argv, code, stderr.String())
	}
	privateDir := strings.TrimSpace(out.String())
	if path.Clean(privateDir) != privateDir || path.Dir(privateDir) != "/tmp" || !strings.HasPrefix(privateDir, privatePushDirPrefix) {
		return "", fmt.Errorf("creating private publication state in sandbox %s returned unsafe path %q: %w", sandbox, privateDir, work.ErrPermanent)
	}
	return privateDir, nil
}

func isolatedGit(args ...string) []string {
	argv := append([]string(nil), isolatedGitEnvironment...)
...
func (s *Sandboxes) runExpecting0(ctx context.Context, sandbox work.SandboxID, op string, argv []string) error {
	...
	code, err := s.repositoryExec(ctx, sandbox, argv, nil, io.Discard, &stderr)
	if err != nil {

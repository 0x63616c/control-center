...
func TestTargetRecoveryReturnsOnlyCanceledPredecessorCheckpoint(t *testing.T) {
	...
	if _, err := fake.ClaimAndStartRun(ctx, store.ClaimRunInput{TicketID: ticket.ID, RunID: runID, StartedAt: time.Now()}); err != nil {
		t.Fatalf("ClaimAndStartRun: %v", err)
	}
	if _, err := fake.StartStep(ctx, store.StartStepInput{RunID: runID, Ordinal: 1, Kind: work.StepCloneRepository, StartedAt: time.Now()}); err != nil {
	...
	if _, err := fake.CheckpointGitEffect(ctx, store.GitCheckpointInput{GitCheckpoint: checkpoint, CompletedAt: time.Now()}); err != nil {
		t.Fatalf("CheckpointGitEffect: %v", err)
	}
	if _, err := fake.CancelRun(ctx, store.CancelRunInput{RunID: runID, TicketID: ticket.ID, EndedAt: time.Now()}); err != nil {

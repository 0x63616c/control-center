...
func (activities *PromptActivities) Prepare(ctx context.Context, input PrepareInput) (PrepareOutput, error) {
	prompt, schema, err := activities.prompts.Render(input.Attempt.Key, input.Attempt.Detail, input.Attempt.Prior, input.Attempt.PromptContext, input.Attempt.LegacyMaxReviewSteps)
	if err != nil {

...
| stage activity attempts | 6 total (5 retries), with 1s/5s/25s/2m5s/5m exponential backoff |
...
The stage activity retry policy allows five retries after the initial attempt,
with exponential backoff, and it applies on the Session's run-specific sandbox
queue. A rollout of the main worker can resume

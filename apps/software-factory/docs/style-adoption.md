...
a Go style guide and 26 ADRs for a codebase maintained by agents. This service adopts a
subset. That repo is still foundation-phase — `internal/clock`, `internal/id` and an empty
`main.go` — so what we imported is **documents, not code**.

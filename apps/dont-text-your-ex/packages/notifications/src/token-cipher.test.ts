...
describe("push token cipher", () => {
  test("round trips with authenticated context and never stores plaintext", () => {
    ...
    const sealed = cipher.seal("aabbccdd00112233aabbccdd00112233", "pdi_device");

    expect(sealed.ciphertext).not.toContain("aabbccdd");
    expect(sealed.keyId).toBe("v1");
    expect(cipher.open(sealed, "pdi_device")).toBe("aabbccdd00112233aabbccdd00112233");
  });

  test("rejects an AAD swap", () => {
    const cipher = createTokenCipher(parseTokenKeyring({ activeKeyId: "v1", keys: { v1: key } }));
    const sealed = cipher.seal("aabbccdd00112233aabbccdd00112233", "pdi_one");
    expect(() => cipher.open(sealed, "pdi_two")).toThrow(/decrypt/);
  });
  ...
  test("reads old keys while all new writes use the active key", () => {
    ...
    const old = oldCipher.seal("aabbccdd00112233aabbccdd00112233", "pdi_device");
    const rotated = createTokenCipher(
      parseTokenKeyring({ activeKeyId: "v2", keys: { v1: key, v2: nextKey } }),
    );
    expect(rotated.open(old, "pdi_device")).toContain("00112233");
    expect(rotated.seal("aabbccdd00112233aabbccdd00112233", "pdi_device").keyId).toBe("v2");
  });

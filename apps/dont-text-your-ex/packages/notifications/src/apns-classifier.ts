...
export function classifyApnsResponse(response: ApnsResponseSummary): ApnsOutcome {
  ...
  if (INVALID_DEVICE_REASONS.has(reason) || response.status === 410) {
    return { kind: "invalid_device", reason };
  }
  if (reason === "TooManyRequests" || response.status === 429) {
    return { kind: "retry", reason, retryAfterMs: response.retryAfterMs ?? 60_000 };
  }
  if (response.status >= 500) {
    return { kind: "retry", reason, retryAfterMs: Math.max(response.retryAfterMs ?? 0, 900_000) };
  }
  if (PROVIDER_CONFIGURATION_REASONS.has(reason) || response.status === 403) {
    return { kind: "provider_configuration", reason };
  }
  return { kind: "permanent_notification", reason };
}

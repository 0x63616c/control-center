...
- Temporal replay tests remain required for command-sequence compatibility,
  but migration of workflow histories from before this redesign is explicitly
...
| S5 | Agent execution activity | Resume versus fresh execution, progress heartbeats, credential rotation, transcript/result reconciliation, and cancellation | Direct activity tests with fake clock, fake Codex process boundary, and temporary filesystem |
...
| S8 | Composition and infrastructure | Correct workflow/activity registration, task queues, pod capabilities, secrets, and GitHub permissions | Composition-root and rendered-infrastructure tests |
...
1. At most one live Run owns a Ticket.
...
| ID | Given | When | Then |
| --- | --- | --- | --- |
| D01 | No dispatcher exists | A worker publishes its resolved policy with Update-With-Start | The dispatcher starts once, accepts the policy, returns `APPLIED`, and only then may that worker poll task queues |
...
| D03 | A dispatcher holds a different policy | A newer worker publishes another valid resolved policy | The accepted latest arrival becomes the policy for future Runs; existing Runs keep their original input snapshot |
...
| W09 | `M1` is a Confirmed Merge | Terminal recording runs | One idempotent transaction stores `M1`, ends the Run successfully, moves the still-owned Ticket to `done`, and satisfies its dependency edges |
...
| ID | Given | When | Then |
| --- | --- | --- | --- |
| F01 | CI for `H1` is pending | `AwaitCI` runs repeatedly | One CI Step remains active; activity retry waits 15 seconds between reads and creates no Agent Attempt or additional Step |
...
| A04 | An agent activity try starts | GitHub credentials were minted earlier | The Run Worker refreshes the installation credential before execution and the agent/Git process uses that current credential |
| A05 | One agent activity try remains active for 30 minutes | Renewal time arrives | The credential is renewed in place without restarting the Agent Attempt or exposing the secret in workflow history |
...
| A11 | A completed agent result exists but the activity response was lost | A retry starts | The activity returns the durable result without running or charging the model again |
...
| ID | Given | When | Then |
| --- | --- | --- | --- |
| P01 | One open Ticket | Two independent claims race | Exactly one returns ownership and one Run can be created |
...
| O05 | The Run Worker pod is rendered | Its security and environment are inspected | It has only the agreed capabilities, no GitHub App private key, no secret in command-line arguments, and one private task queue identity |
...
6. Textual merge-conflict feedback (`F04`) and changed-head invalidation
   (`F05`).
7. Agent Attempt recovery and total budget (`A01`-`A12`, `F11`, `F12`).
8. Cancellation and irreversible finalization (`C01`-`C08`).
9. Retry-backed dispatcher wait and acknowledged policy publication
   (`D01`-`D07`).
10. Drained Continue-As-New and child cancellation ownership (`D08`-`D12`).
11. Real-Temporal Run Worker Session affinity (`I01`-`I07`).
12. Console projection, webhook retirement, and security assertions
    (`O01`-`O08`).

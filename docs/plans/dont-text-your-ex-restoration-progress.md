...
| Create, join, invite, deep links | recovery/frontend agents + root | v1 audit agents | Create → Invite → Jar pointer path green; `3720d8609`/`456e5b619` add canonical URL, onboarding-preserved web path, strict pre-join boundary, Capacitor cold/warm plumbing, AASA and 11th browser flow; immutable run `31866340124` green; `f03617886` adds seven-day absolute invite expiry, owner-only confirmed replacement, immediate old-code revocation, and real-Postgres reload coverage | Public AASA/Apple CDN and physical cold/warm universal-link proof pending; hosted replacement/deep-link browser row pending | Blocked — invite lifecycle fixed |

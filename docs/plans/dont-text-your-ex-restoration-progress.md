...
| Preview and join jar | recovery/frontend agents + `release_spec_rereview` | `backend_release_qa` | `3720d8609`/`456e5b619` implement strict preview/deep-link join; `f40344ec2` adds invalid code, 403/409/5xx/offline/retry states. Spec rereview found a time-of-check/time-of-use race could admit a member after close or rotation invalidated the code. `742834f79` makes join lock/revalidate the invite and commit membership/activity atomically; controlled raw HTTP close/rotate races prove old-code join 404 and no membership. `35da52864` replaces nullable preview/error/busy combinations with explicit preview and join states while preserving the Storybook 5xx/offline retry flow. Independent Linux Node 26.7.0 + Postgres 17 full store execution passed 28/28 at the fixing commit | Defects fixed and independently retested; immutable hosted run `31869366940` and physical deep-link proof remain pending | Blocked — code/independent Postgres pass; hosted/physical proof pending |
...
| Create named or anonymous report | reporting agents + `release_standards_rereview` | v1 audit agents | Anonymous reporter redaction is covered; `eac6277ee` rejects self-reporting. `35da52864` models report-member loading/error/loaded and idle/submitting/failed/sent states, exposes fetch/send retry, preserves form input after failure, and adds a Storybook interaction proving failures never show “Snitched” before a successful retry | Frontend typecheck and focused Biome/pre-commit pass; immutable hosted run `31869366940` and independent named/anonymous authorization QA remain pending | Blocked — hosted/independent proof pending |
...
| Loading, error, validation, and empty-state coverage | feature owners + `release_standards_rereview` | v1 audit agents | `7ea259720`/`f8df0df1d`, `f40344ec2`, `4a77eb0f6`/`55568464c`, and `bdec66cee` cover the targeted fetched and mutation states across core screens; `35da52864` closes the remaining Join and Report Member boolean/null state gaps, adds visible retry without false success, and passes native `disabled` semantics through the shared button | Authored seams/local gates exist; immutable hosted run `31869366940`, independent hands-on visual QA, and production network behavior remain pending | Blocked — hosted/independent proof pending |

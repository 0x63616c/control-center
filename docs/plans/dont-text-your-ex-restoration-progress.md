...
| Create jar | recovery/frontend agents | v1 audit agents | Immutable run `31866340124` proves the earlier happy path at `456e5b619`; `f40344ec2` adds offline/retry and duplicate-submit protection | Later failure-state commit lacks completed immutable and independent feature QA; production persistence is unproved | Blocked — later states need hosted/independent/production proof |
| Preview and join jar | recovery/frontend agents | v1 audit agents | `3720d8609`/`456e5b619` implement strict preview/deep-link join; `f40344ec2` adds invalid code, 403/409/5xx/offline/retry states | Earlier path is green in `31866340124`; later failure states and current authorization behavior need hosted/independent proof | Blocked — later states need hosted/independent proof |

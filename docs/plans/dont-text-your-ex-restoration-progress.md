...
Reconciled at `ad939cb77`. The latest completed fully green immutable run is
`31866340124` at `456e5b619`, before the feature commits below. Current-head runs
`31867953183` and `31867955530` were queued/pending during this reconciliation;
they are not recorded as green evidence. Superseded intermediate runs were
cancelled and likewise do not prove their feature rows.

| Feature | Implementation owner | Independent QA | Automated/hands-on evidence | Defects or missing states | Result |
|---|---|---|---|---|---|
| Apple auth, onboarding, profile/avatar | `apple_release`, `fix_ci_apple_arch` | v1 audit agents | Apple verifier/unit seams, onboarding/profile browser paths, and unsigned simulator build; `dc422b50b`/`583b06ec1` enforce avatar PNG/JPEG/WebP signature, MIME, and 2 MiB boundaries plus save/render/reload browser assertions; `ad939cb77` records regenerated Apple profile state | Avatar implementation/local push gate are green, but current-head hosted execution and independent hands-on profile QA are pending. Physical Apple success/cancel/reload/cellular, signed Associated Domains, and TestFlight-device proof remain absent | Blocked — implemented locally; hosted, independent, and physical proof pending |
| Create, join, invite, deep links | recovery/frontend agents + root | v1 audit agents | `3720d8609`/`456e5b619` add canonical web/native deep links, onboarding preservation, AASA, strict pre-join summaries, and 11 browser flows; immutable run `31866340124` is green at `456e5b619`. `f40344ec2` adds Create/Join/Invite fetch, validation, offline, duplicate-submit, and retry states. `f03617886`/`bdec66cee` add seven-day expiry, owner confirmation/retry UI, immediate rotation revocation, Storybook, reload, and old/new deep-link Playwright; `463e89e73` fixes seeded invite expiry | Later lifecycle/failure-state commits passed local typecheck and push gates plus direct real-Postgres proof, but current-head hosted and independent feature QA are pending. Public AASA/Apple CDN and physical iOS cold/warm universal-link proof remain absent | Blocked — implemented locally; hosted, independent, and physical proof pending |
...
| Self-log slip | recovery/frontend agents | v1 audit agents | Amount/confirm/tally/pot/streak flow and private-ex-label isolation; `f40344ec2` adds initial fetch failure, submit failure without false success, offline/retry, duplicate-submit guard, and reload browser coverage | Implementation, frontend typecheck, and push gate are green. Hosted privacy/failure-state execution, independent hands-on mobile QA, and production proof are pending | Blocked — failure states implemented locally; hosted/independent QA pending |
| Reports, anonymity, real images, evidence, own/deny/history | reporting agents + `fix_ci_apple_arch` | v1 audit agents | Real PNG/JPEG/WebP evidence contracts and Postgres round trip; anonymous activity redaction and self-report rejection; `fe8f93565`/`4a77eb0f6` add durable owned/denied history, active-member/outsider isolation, activity links, reload-safe detail, loading/error/empty Storybook, and Playwright; `81555750b` preserves closed-jar report history; `55568464c` adds report-history state QA; `205863490` records the feature audit | Implemented with local typecheck/push-gate and authored Postgres/Storybook/Playwright seams. Runs at `fe8f93565` failed and therefore are not accepted as feature evidence; later runs were superseded. Current-head hosted execution and independent hands-on image/deny/history QA remain pending | Blocked — history implemented locally; hosted/independent QA pending |
| Settle, close, and leave lifecycle | recovery/frontend agents | v1 audit agents | `7ea259720` prevents false `$0` and adds retry; `725271799`/`23e24e14d` persist and correctly deserialize closure; `a0e0c0ea5` preserves history on leave; `c34b27741` adds confirmed owner close/member leave UI, Storybook, and Playwright; `e324710c7` isolates departed members; `900985fc7` scopes settle retry amount | Local real-Postgres authorization/reload seams and push gates are green. Hosted close/leave/settle execution, independent loaded positive/zero and read-only-history hands-on QA, and production persistence proof remain pending | Blocked — lifecycle implemented locally; hosted/independent/production proof pending |
| Sessions and logout | auth agents | v1 audit agents | `72dd17844` proves 30-day absolute expiry, concurrent sessions, lazy expiry deletion, non-sliding last-used observation, and current-token logout against real Postgres/Hono; `acec8fc8a` preserves sessions through network/5xx/invalid payloads, clears only confirmed 401, retries reload, and revokes before local logout | Direct/unit seams and local push gates are green. No later immutable hosted run proves the combined backend/frontend lifecycle; independent browser QA, physical-device presentation, and production account/session persistence remain pending | Blocked — implemented locally; hosted/independent/production proof pending |
| Loading, error, validation, empty | feature owners | v1 audit agents | `7ea259720`/`f8df0df1d` cover Home, Activity, Jar Detail, Settle, Confirm/Deny, and failed report mutation; `f40344ec2` adds Create, Invite, Join, and Log Slip validation/offline/retry/no-false-success states; `4a77eb0f6`/`55568464c` cover report-history loading/error/empty/detail states; `bdec66cee` adds invite-rotation failure/retry without losing the current code | Screen seams and local typecheck/push gates are implemented. Current-head hosted Storybook/Playwright execution, independent hands-on mobile/visual QA, and production network-failure behavior remain pending | Blocked — targeted states implemented locally; hosted/independent QA pending |
| Mobile accessibility and clickability | `fix_ci_apple_arch` | v1 audit agents | `506dacad0` names 44px icon/avatar/evidence/back controls, labels switches, and implements viewer focus trap/Escape/return; `182fc9c46` aligns accessible browser locators; `c9cf37afb` covers subpixel touch rounding; `744f41dad` records 320px touch/keyboard QA. Frontend typecheck and full Biome/Knip push gates are green | Authored Storybook/Playwright seams are not yet green at a later immutable SHA. Physical VoiceOver announcements/rotor, rotation, Dynamic Type, and device touch QA remain absent | Blocked — automated implementation complete; hosted/device QA pending |
...
| Authorization and data isolation | `fix_frontend_quality` + auth/report agents | v1 audit agents | Earlier multi-user seams cover hidden streak, ex labels, anonymous reporters, opt-in defaults, and self-target rejection. `cc826c230` adds the complete raw API/Postgres owner, active-member, accused, former-member, and outsider matrix across jar read, invite preview/join, slip, streak, report create/read/resolve/history, close, and leave; hidden/nonexistent bodies match and `e324710c7` isolates departed history | Matrix implementation and direct real-Postgres spot proof are green, but the `cc826c230` hosted runs were superseded/cancelled. Current-head hosted execution and an independent matrix reconciliation remain pending; production isolation is unproved | Blocked — full matrix implemented locally; hosted/independent/production proof pending |

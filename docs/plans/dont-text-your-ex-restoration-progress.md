...
| `bbd4bf530`, `bfea18405` | Cryptographic invite codes and bounded probe throttling | Six-character invite UX now uses uniform `node:crypto` selection rather than `Math.random`. Authenticated preview and join share a 20-token/minute limiter with independent user and Cloudflare/forwarded client-IP buckets, typed `429 invite_rate_limited` JSON, `Retry-After`, frontend retry timing, and one-minute idle-bucket collection. Raw HTTP QA exhausts and crosses both sources, proves preview/join share the budget, and retains the normal valid join response. The unverified Apple subject is no longer logged. API/frontend typechecks, focused/full Biome, Knip, pre-commit, and pre-push passed; hosted runtime execution is pending |
| `50603bd4d` | Complete invite join test harness | Exact-head hosted run `31871067483` executed 75 product tests: 74 passed and the new normal-join assertion alone failed because the isolated route-boundary store mock omitted `joinJarByCode`, producing a test-only 500. The harness now supplies the same explicit null/not-found seam as preview; production code is unchanged. API typecheck, focused/full Biome, Knip, pre-commit, and pre-push passed. Focused local Vitest emitted no output and was terminated after the bounded 90-second timeout; immutable hosted retest is pending |
...
| Apple issuer, audience, signature, nonce, and state validation | `apple_release`, `/root` | `qa_auth_profile`, `final_ship_standards` | Independent source audit confirmed `jwtVerify` pins Apple issuer and `co.worldwidewebb.textyourex`, hashes and checks the raw nonce, and the native seam binds both random attempt ID and state. Focused unit cases cover valid exchange plus wrong issuer, audience, signature, nonce, missing nonce, substituted/missing state, replayed attempt ID, and schema-invalid native responses. Production errors omit verifier detail; logs include neither the identity token nor the unverified stable Apple subject | Local Vitest entered the known silent host stall and was stopped; exact-commit hosted execution and a real Apple token exchange remain absent | Automated PASS pending hosted confirmation; live Apple Blocked |
...
| Copy/share invitation and complete invite screen | `fix_frontend_quality`, `/root` | `qa_jars_evidence` | Independent browser QA covers the fresh-create completion button, reload into the created jar, Share fallback with visible copy confirmation, Done returning to the existing jar, and reopening Invite. Storybook separately covers direct code copy, expiry display, missing/expired disabled share, owner replace retry, and a member who can share but cannot replace | Native iOS share-sheet presentation/dismissal and physical clipboard receipt remain untested | Automated PASS pending exact-head hosted confirmation; physical Blocked |
...
| Invalid, expired, unauthorized, or throttled join failures | `/root`, `final_ship_standards` | `final_ui_qa`, `final_ship_standards` | `b2fa4fc4e` covers invalid input and 404/403/409/5xx/offline preview failures, hidden stale content, successful retry, and absence of fake seeded codes. `bbd4bf530` preserves six-character codes while moving generation to uniform OS cryptographic randomness; raw HTTP QA proves a shared 20/min preview/join budget cannot be bypassed by changing only account or IP, returns typed 429 plus `Retry-After`, and leaves a normal valid join response unchanged. The frontend presents the typed retry delay | Exact-commit hosted execution and deployed edge-header behavior remain pending | Automated PASS pending hosted/production confirmation |
| Invitation expiry, revocation, and closed-jar old links | `fix_frontend_quality`, `/root` | `qa_jars_evidence` | Independent Postgres/API audit covers explicit owner confirmation, member 403, former/outsider 404, replacement expiry greater than six and no more than seven days, old preview/join 404 for every role, one-millisecond-past-expiry preview/join 404, replacement surviving a fresh read, closed replacement 409, and serialized join versus close/rotation races. Browser QA rotates, proves the old deep link stays 404 after reload, proves the new link still previews, and separately closes a jar then verifies old invite 404 plus mutation 409. Storybook covers expiry/unverifiable UI and failed replacement preserving the current code | Physical universal-link launch at the seven-day boundary and deployed clock/production behavior remain untested | Automated PASS pending exact-head hosted confirmation; physical/production Blocked |
...
| Return to jar after creation/invitation | `/root` | `qa_jars_evidence` | Independent browser QA uses the fresh Invite screen's Take me to my jar action, reloads and finds the created jar, uses existing Invite's Done action to return to jar detail, and completes a second-user join back to the populated home/jar. Reload and fresh-route behavior are covered without synthetic navigation calls | Physical iOS back gesture and suspended-app restoration remain untested | Automated PASS pending exact-head hosted confirmation; physical Blocked |
| Log a self slip with required validation | `/root` | `qa_jars_evidence` | Independent contract audit rejects invalid amounts. Genuine-pointer browser QA changes the default amount, confirms through the friction sheet, observes pot/tally growth, and separately induces jar-fetch 503 and slip-submit 503: no false success or tally claim appears, retry submits once, and a full reload reads the jar again. Real-Postgres privacy tests prove private ex labels never enter shared jar/activity JSON and outsider/former slip writes remain 404 | API/database pod-restart durability, physical mobile controls, and production persistence remain untested | Automated PASS pending exact-head hosted confirmation; physical/production Blocked |
...
| Note-or-image invariant at UI, contract, and API | `/root` | `qa_jars_evidence` | `986089d5c` adds the independent empty, whitespace-only, note-only, image-only, and exact-three-image matrix. The UI keeps Send disabled for empty/whitespace-only input; the shared API schema rejects both and accepts note or valid evidence. Hosted run `31870452811` passed the Postgres/API contracts but exposed that whitespace followed by valid images was sent untrimmed and rejected by the API; `5265e1a4b` normalizes it to an image-only request and the same browser case is retained for retest | Fresh hosted browser retest at the fixing commit is pending; physical and production execution remain absent | Defect fixed; automated retest pending, physical/production Blocked |
| PNG/JPEG/WebP browser and iOS picker | `/root` | `qa_jars_evidence` | `986089d5c` drives the real hidden file input through browser `FileReader` with one PNG, JPEG, and WebP in the same submission, then requires three rendered attachments and a successful API-backed report. The shared-schema matrix independently accepts each matching signature | The first hosted execution reached the API but was blocked by the whitespace defect fixed in `5265e1a4b`; fresh hosted retest and a physical iOS WebView picker run remain pending | Automated retest pending; physical Blocked |
| Evidence maximum three images and 2 MiB each | `/root` | `qa_jars_evidence` | `986089d5c` proves exactly three attachments and an exactly 2 MiB signature-valid image are accepted, four files are rejected by the browser with zero thumbnails added, and 2 MiB plus one byte is rejected at both the client/shared boundary. The browser then submits the exact-three format set | Fresh hosted browser retest at the fixing commit and physical memory-pressure behavior remain pending | Automated retest pending; physical Blocked |
| Evidence MIME, signature, count, and size validation | `/root` | `qa_jars_evidence` | Independent malicious-boundary coverage in `986089d5c` rejects MIME/data-URL mismatch, valid MIME with wrong magic bytes, malformed base64, GIF, oversize, and excess count; browser QA uploads a file named/type-labelled PNG with non-image bytes, gets a read error, and adds zero thumbnails. Hosted `31870452811` passed the expanded shared/API contract suite before its later browser failure | Fresh fixing-commit browser execution and deployed raw-API probes remain pending | Contract PASS; browser retest/production Blocked |
...
| Real thumbnails and full evidence viewer | `/root`, `fix_ci_apple_arch` | `qa_jars_evidence` | Independent Storybook/browser audit verifies real data-URL thumbnails, labelled full-screen dialog, initial close-button focus, Escape close, and focus return in both shared and resolved-history states. It found an unsent composer thumbnail exposed as an enabled View button with no handler; `5ee9aadf3` renders handler-less thumbnails non-interactively and adds a regression assertion while persisted evidence remains actionable | Fresh hosted Storybook execution at the fixing commit, physical image rendering, and production evidence retrieval remain pending | Defect fixed; automated retest pending, physical/production Blocked |
...
| Home and activity: empty and populated | `/root` | `qa_auth_profile`, `qa_jars_evidence` | Independent browser QA covers a first-run no-jars account with Create/Join actions, reload of the same empty state, seeded populated home with two named jars and aggregate damage, populated jar detail, and populated activity ending honestly. Storybook covers loading, failed fetch, Retry, and true empty for both Home and Activity, without false zero/empty content during loading | Fresh exact-head hosted execution and physical mobile visual QA remain pending | Automated PASS pending hosted confirmation; physical Blocked |
| Core actions reachable without dead ends | `/root` | `final_ui_qa`, `qa_jars_evidence` | Independent genuine-pointer sweep reaches onboarding, Create, Join, jar detail, Invite/share/completion, Log Slip/confirm, Report/send, Activity/report history, Profile, Settle, Close, Leave, and rejoin/reload paths, including retryable 403/409/5xx/offline states. The sweep found the inert unsent-evidence View control and fixed/retested its rendered semantics in `5ee9aadf3`; `5667732db` adds Share fallback and Done-to-jar browser coverage | Fresh fixing-commit hosted browser/Storybook execution and physical iOS back/touch QA remain pending | Defect fixed; automated retest pending, physical Blocked |

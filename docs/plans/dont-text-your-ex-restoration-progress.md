...
| Apple issuer, audience, signature, nonce, and state validation | `apple_release`, `/root` | `fix_pr_browser_ci/independent_v1_audit` (source audit only) | `c8560f331` binds nonce/state; Apple JWT tests cover issuer/audience/signature/nonce | Exact-head hosted execution and real Apple token exchange remain absent | Blocked — hosted/live proof pending |
| Returning Apple user without `fullName` | `/root` | Unassigned | `3056f4190` and Apple account unit tests retain the existing account when Apple omits the name | No named independent or production-returning-user QA | Blocked — independent/live proof pending |
| New Apple user without an Apple-provided name | `/root` | Unassigned | `3056f4190` persists an empty name and routes to user-entered setup; `6d63b3c88` validates fixtures | No named independent or production first-run QA | Blocked — independent/live proof pending |
| Development authentication disabled in production | `app_recovery` | Unassigned | Production boot/API contracts assert `/auth/dev` and `/test/reset` return 404; `79824febc` modernizes runtime config | Exact-head hosted and deployed-production requests remain absent | Blocked — independent/production proof pending |
| Session creation, persistence, expiry, and logout/revocation | `/root`, `release_standards_rereview` | Unassigned | `72dd17844`, `acec8fc8a`, `9536caa99`, and `35da52864` cover absolute expiry, independent sessions, transient restore, confirmed 401, revocation, and branded tokens | No named independent browser or production persistence QA | Blocked — independent/production proof pending |
| First-run onboarding | `/root` | Unassigned | `3056f4190`, `6d63b3c88`, and onboarding Storybook seams cover required user-entered naming | No named independent reload/error or production Apple-backed QA | Blocked — independent/live proof pending |
| Later profile and avatar editing | `fix_ci_apple_arch` | Unassigned | `dc422b50b`/`583b06ec1` cover edit, reject, save, render, and reload | No named independent hands-on physical or production persistence QA | Blocked — independent/physical/production proof pending |
| Avatar MIME, signature, and size limits | `fix_ci_apple_arch` | Unassigned | `dc422b50b`/`583b06ec1` enforce PNG/JPEG/WebP signatures and the 2 MiB client/server limit | Exact-head hosted and named independent picker QA absent | Blocked — independent/hosted proof pending |
...
| Useful empty-account state | `/root` | Unassigned | `7ea259720`, `e77c5784a`, and `ce2bccf34` cover loading, true empty, loaded, failure, retry, and fetched timing | Named independent populated/empty mobile QA absent | Blocked — independent/hosted proof pending |
...
| Invalid, expired, or unauthorized join failures | `/root` | `final_ui_qa` | `b2fa4fc4e` covers invalid input and 404/403/409/5xx/offline preview failures, hidden stale content, and successful retry | Immutable hosted execution absent | Blocked — hosted proof pending |
...
| Core actions reachable without dead ends | `/root` | `final_ui_qa` (focused report/join audit) | `28c2dacaa` restores invite completion; `b28613dde` fixes pointer interception; `b2fa4fc4e` removes former report targets and stale/fake join states | Full named independent navigation sweep and physical QA absent | Blocked — independent/physical proof pending |
| Settle display and close-jar lifecycle | `/root` | Unassigned | `7ea259720`/`900985fc7` keep settle honest/inert; `725271799`, `23e24e14d`, and `c34b27741` persist confirmed owner close and read-only history | Named independent owner/member hands-on QA and production restart/backup proof absent | Blocked — independent/production proof pending |
| Leave-jar lifecycle and historical membership | `/root` | `final_ui_qa` (report-target boundary) | `a0e0c0ea5`, `c34b27741`, `e324710c7`, and `b2fa4fc4e` preserve history, isolate former access/counts, mark inactive rows, reject/filter former report targets | Full independent leave/rejoin/reload and production proof absent | Blocked — hosted/production proof pending |
| Honest loading, error, validation, offline, expired-session, and empty states | `/root`, `release_standards_rereview` | `final_ui_qa` (join/report subset) | `7ea259720`, `f8df0df1d`, `f40344ec2`, `acec8fc8a`, `35da52864`, and `b2fa4fc4e` cover discriminated recovery states, no false success, preserved form data, and exhaustive labels | Full named independent hands-on visual sweep, exact-head hosted execution, and production network behavior absent | Blocked — independent/hosted/production proof pending |
| Mobile accessibility and navigation | `fix_ci_apple_arch` | Unassigned | `506dacad0`, `182fc9c46`, `c9cf37afb`, and `744f41dad` cover labelled 44px controls, switches, viewer focus, Escape/return, and 320px automation | Physical VoiceOver, rotor, rotation, Dynamic Type, and touch QA absent | Blocked — named physical QA absent |

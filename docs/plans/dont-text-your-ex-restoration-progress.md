...
| Preview and join jar | recovery/frontend agents + `release_spec_rereview` | `backend_release_qa` + `final_ui_qa` | `3720d8609`/`456e5b619` implement strict preview/deep-link join; `f40344ec2` adds invalid code and submit recovery states. Spec rereview found a time-of-check/time-of-use race could admit a member after close or rotation invalidated the code. `742834f79` makes join lock/revalidate the invite and commit membership/activity atomically; controlled raw HTTP close/rotate races prove old-code join 404 and no membership. `35da52864` replaces nullable preview/error/busy combinations with explicit states. `b2fa4fc4e` independently adds the missing browser matrix for invalid local input and 404 expired, 403, 409, 5xx, offline preview failures, proves each failure hides preview content and a final retry recovers, uses exhaustive button labels, and removes the production fake-code hint | Local code review/Biome/typechecks pass; immutable hosted execution at `b2fa4fc4e` and physical deep-link proof remain pending | Blocked — code/independent QA pass; hosted/physical proof pending |
...
| Create named or anonymous report | reporting agents + `release_standards_rereview` | `final_ui_qa` | Anonymous reporter redaction is covered; `eac6277ee` rejects self-reporting. `35da52864` models report-member fetch/mutation states. Independent QA found historical former members remained selectable even though the API rejected them, and success presentation could read mutable target/anonymity state. `b2fa4fc4e` adds an active-membership DTO bit while retaining historical rows, proves raw former-target rejection, filters inactive targets, snapshots submitted target/anonymity, disables target/note/evidence/anonymous controls in flight, and adds Storybook/browser regressions proving 401/403/5xx/offline fetch retry, failed-send preservation of note/image/anonymous/target, no false success, and successful retry with the correct target/anonymity | Defects fixed and local Biome/typechecks/pre-commit pass; immutable hosted execution at `b2fa4fc4e`, production authorization, and physical image-flow QA remain pending | Blocked — independent code QA pass; hosted/production/physical proof pending |
...
| Resolve report outcomes | reporting agents | `backend_release_qa` backend | Own/deny behavior and failed-mutation recovery are covered by report seams and `f8df0df1d`. `55c1c8894` proves raw authenticated accused-only authorization for both actions, 404 for the accuser, 404 on terminal retries, recovery after an induced 500, and fresh detail/history persistence. Linux Node 26.7.0 + Postgres 17 passed 30/30 | Backend normal, authorization, failure, retry, and reload states pass independently. Hosted browser interaction and production persistence remain pending | Blocked — independent backend pass; hosted UI/production proof pending |
...
| Report history and evidence detail | reporting agents + `fix_ci_apple_arch` | `backend_release_qa` backend | `fe8f93565`/`4a77eb0f6` add durable history/detail, activity links, reload and state stories; `81555750b` preserves closed-jar history; `55568464c` adds state QA; `205863490` records audit. `55c1c8894` proves fresh authenticated history/detail reads retain owned and denied outcomes for both active viewers without private profile values after failure recovery and reload | Backend resolved-history persistence/privacy passes independently. Current-head hosted UI/evidence rendering, independent hands-on image QA, and production backup/restore remain pending | Blocked — independent backend pass; hosted UI/production proof pending |
...
| Leave jar | recovery/frontend agents | `final_ui_qa` reporting boundary | `a0e0c0ea5` preserves history; `c34b27741` adds confirmed member UI; `e324710c7` isolates departed members. Independent QA found the history-preserving detail DTO did not distinguish active from departed rows, allowing Report Member to offer an invalid former target. `b2fa4fc4e` serializes active status, retains the former member/tally in history, raw-asserts former false and owner true, rejects reporting the former target, and filters that target from the UI | Reporting-boundary defect fixed with raw/UI regressions; immutable hosted execution and production leave/reload proof remain pending | Blocked — code/independent boundary QA pass; hosted/production proof pending |
| Sessions and logout | auth agents + `release_standards_rereview` | v1 audit agents | `72dd17844` proves absolute expiry/concurrency/logout in real Postgres/Hono; `acec8fc8a` covers transient failures, confirmed 401, reload, and revoke-before-clear; `9536caa99` models restore states; `35da52864` preserves the parsed `SessionToken` brand through `setToken`, `AppCtx.signIn`, and `App.signIn`, with compile-time rejection of plain strings | Direct/local seams and frontend typecheck pass; immutable hosted run `31869366940`, independent browser, physical presentation, and production account/session persistence remain pending | Blocked — hosted/independent/production proof pending |
...
| Loading, error, validation, and empty-state coverage | feature owners + `release_standards_rereview` | `final_ui_qa` focused frontend | `7ea259720`/`f8df0df1d`, `f40344ec2`, `4a77eb0f6`/`55568464c`, and `bdec66cee` cover core states; `35da52864` introduces explicit Join/Report Member state unions and native shared-button disabling. `b2fa4fc4e` independently closes the remaining focused gaps: exhaustive labels, invalid/404/403/409/5xx/offline preview states with hidden stale content and recovery, report fetch 401/403/5xx/offline retry, full failed-send form preservation, immutable success snapshot, all in-flight input disabling, and the hosted Storybook nested-alert locator correction | Focused defects fixed; local Biome/typechecks/pre-commit pass. Immutable hosted run at `b2fa4fc4e`, independent hands-on visual QA, and production network behavior remain pending | Blocked — independent code QA pass; hosted/visual/production proof pending |

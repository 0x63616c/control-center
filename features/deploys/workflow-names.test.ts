...
describe("deploys temporal facet workflow type names", () => {
  ...
  it("registers the activity the workflow proxies", () => {
    ...
    expect(activitiesSource).toMatch(/export async function DeploysPurgeActivity\b/);
    expect(workflowsSource).toMatch(/const \{ DeploysPurgeActivity \} = proxyActivities/);
  });
});

...
describe("weather temporal facet workflow type names", () => {
  ...
  it("registers the activity the workflow proxies", () => {
    ...
    expect(activitiesSource).toMatch(/export async function WeatherPurgeActivity\b/);
    expect(workflowsSource).toMatch(/const \{ WeatherPurgeActivity \} = proxyActivities/);
  });
});

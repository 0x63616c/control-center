...
describe("felogs temporal facet workflow type names", () => {
  ...
  it("registers the activity the workflow proxies", () => {
    ...
    expect(activitiesSource).toMatch(/export async function FelogsPurgeActivity\b/);
    expect(workflowsSource).toMatch(/const \{ FelogsPurgeActivity \} = proxyActivities/);
  });
});

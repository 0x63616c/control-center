 ...
 *
 * `interaction-session-service` (apps/api) and this feature's own db both
 * resolve `config.DATABASE_URL` to the same connection string, so
 * `createFeatureDb` memoizes them onto the SAME underlying pool — one shared
 * pool against the physical `frontend_log` table from the same process tree,
 * not two.
 */

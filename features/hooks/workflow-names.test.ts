...
describe("hooks temporal facet workflow type names", () => {
  ...
  it("registers the activity the workflow proxies", () => {
    ...
    expect(activitiesSource).toMatch(/export async function HooksPurgeActivity\b/);
    expect(workflowsSource).toMatch(/const \{ HooksPurgeActivity \} = proxyActivities/);
  });
});

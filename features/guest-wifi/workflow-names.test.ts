...
describe("guest-wifi temporal facet workflow type names", () => {
  ...
  it("registers the activity the workflow proxies", () => {
    ...
    expect(activitiesSource).toMatch(/export async function GuestWifiPurgeActivity\b/);
    expect(workflowsSource).toMatch(/const \{ GuestWifiPurgeActivity \} = proxyActivities/);
  });
});
